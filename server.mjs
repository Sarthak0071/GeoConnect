...
//   const app = express();
//   const PORT = process.env.PORT || 5000;
//   const CACHE_TTL = process.env.CACHE_TTL || 2592000;
//   const POPULAR_CITIES = ["Kathmandu", "Pokhara", "Chitwan", "Lumbini", "Bhaktapur"];

//   const groq = new Groq({
//     apiKey: process.env.GROQ_API_KEY || "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" // Replace with your actual key
//   });
      ...
      for (let i = 0; i < MAX_CONCURRENT_BATCHES; i++) {
        ...
        const batchPromise = Promise.all(batch.map(async (item) => {
          try {
            ...
            const result = await groq.chat.completions.create({
              model: "mistral-saba-24b", // Updated to mistral-saba-24b
              messages: [{ role: "user", content: prompt }],
              temperature: 0.2,
              max_tokens: 512
            });
