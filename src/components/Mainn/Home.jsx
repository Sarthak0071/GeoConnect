...
//   const [showAboutMe, setShowAboutMe] = useState(false); // State for "About Me" visibility
...
const Home = () => {
  ...
  const fetchTouristPlacesFromServer = async (city) => {
    try {
      ...
      if (data.places) {
        ...
        setTouristPlaces(
          placesWithCoordinates.filter((place) => place.location !== null)
        );
      }
    } catch (err) {
      console.error("Error fetching tourist places:", err);
  ...
  return (
    <div>
      <h1>Welcome to the Tourism Map</h1>
      <button onClick={() => navigate("/user-profile")}>About Me</button> {/* Navigate to UserProfile */}
      <LocationControls
        mainCity={mainCity}
        isChangingLocation={isChangingLocation}
        setIsChangingLocation={setIsChangingLocation}
