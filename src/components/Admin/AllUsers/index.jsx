...
import { useUsers, useFilteredUsers } from "../AdminUtils/userHooks";
import { deleteUser, toggleBanStatus } from "../AdminUtils/userFunctions";
import { useCurrentAdmin } from "../AdminUtils/adminHooks"; // Added import
import "./AllUsers.css";

const AllUsers = () => {
  const { currentAdmin } = useCurrentAdmin(); // Added hook
  const { users, setUsers, loading, error } = useUsers("all");
  const [filter, setFilter] = useState("all");
  const [searchQuery, setSearchQuery] = useState("");
  const filteredUsers = useFilteredUsers(users, filter, searchQuery);
  ...
  const confirmDelete = () => {
    deleteUser(currentAdmin, selectedUser, setUsers, setShowDeleteModal, setSelectedUser)
      .catch(err => alert(err.message));
  };

  const confirmBan = () => {
    toggleBanStatus(currentAdmin, selectedUser, banReason, setUsers, setShowBanModal, setSelectedUser, setBanReason)
      .catch(err => alert(err.message));
  };
