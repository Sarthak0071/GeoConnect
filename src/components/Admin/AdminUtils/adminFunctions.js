...
export const addAdmin = async (newAdmin, currentAdmin, setAdmins, setShowForm, setFormError) => {
  try {
    ...
    // Add user data to Firestore with firstLogin flag
    await setDoc(doc(db, "users", newUserId), {
      name: newAdmin.name,
      email: newAdmin.email,
      role: "admin",
      firstLogin: true, // Add this flag to indicate it's their first login
      createdAt: serverTimestamp(),
      createdBy: currentAdmin?.id || "system"
    });

    // Send password reset email (optional - can be removed if you're using the custom first login flow)
    // await sendPasswordResetEmail(tempAuth, newAdmin.email);
   
    // Update the UI state with the new admin
    const newAdminData = {
      id: newUserId,
      name: newAdmin.name,
      email: newAdmin.email,
