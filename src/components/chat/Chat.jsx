...
const Chat = () => {
  const { userId: otherUserId } = useParams(); // userId is optional
  ...
  useEffect(() => {
    ...
    const unsubscribeChats = subscribeToChats(auth.currentUser.uid, (updatedChats) => {
      ...
      if (!currentChat && filteredChats.length > 0 && !otherUserId) {
        setCurrentChat(filteredChats[0]);
      }
    });
    return () => unsubscribeChats();
  }, [blockedUsers, currentChat, otherUserId]);

  const handleChatSelect = (chat) => {
    setCurrentChat(chat);
    if (!otherUserId && chat.otherUserId) {
      navigate(`/chat/${chat.otherUserId}`); // Update URL when selecting a chat
    }
  };
